package com.mozturk.survivorbird;
		...
		if(gameState == 1) {
			...
			for(int i=0; i<numberOfEnemies; i++){
				...
				if(enemyX[i] < Gdx.graphics.getWidth()/15){
					...
					enemyOffset[i] = (random.nextFloat()-1.2f) * (Gdx.graphics.getHeight()-200);
					enemyOffset2[i] = (random.nextFloat()-1.2f) * (Gdx.graphics.getHeight()-200);
					enemyOffset3[i] = (random.nextFloat()-1.2f) * (Gdx.graphics.getHeight()-200);
				...
				for(int a=0; a<lives;a++) {
					batch.draw(live, Gdx.graphics.getWidth()/10*a*0.5f-1f, Gdx.graphics.getHeight()-70f, 30f, 30f);
				}
